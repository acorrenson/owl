...
let _ =
  ...
  let* r3 = do_parse parse_stmt "nateq(z, z)." in
  let* r4 = do_parse parse_stmt "nateq(sc(?x), sc(?y)) :- nateq(?x, ?y)." in
  let* q1 = do_parse parse_fun "sm(sc(z), z, ?r)" in
  let* q2 = do_parse parse_fun "sm(sc(z), sc(z), ?r)" in
  let* q3 = do_parse parse_fun "nateq(sc(z), sc(sc(z)))" in
  let db = [r1; r2; r3; r4] in
  let ll1 = solve_one q1 db in
  let ll2 = solve_one q2 db in
  let ll3 = solve_one q3 db in
  Some (print_sols q1 ll1; print_sols q2 ll2; print_sols q3 ll3)